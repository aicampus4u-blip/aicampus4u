...
const CustomBotInputSchema = z.object({
  botType: z
    .enum(["Field", "Profession", "Topic"])
    .describe("The type of the bot: Field, Profession or Topic."),
  field: z
    .string()
    .optional()
    .describe(
      "The academic field of study for the bot (e.g., Astrophysics). Required if botType is Field."
    ),
  profession: z
    .string()
    .optional()
    .describe(
      "The career for the bot (e.g., Graphic Designer). Required if botType is Profession."
    ),
  topic: z
    .string()
    .optional()
    .describe(
      'The topic for the bot (e.g., "The History of Rome"). Required if botType is Topic.'
    ),
  description: z
    .string()
    .describe("A detailed description of the bot and its intended purpose."),
});
...
const CustomBotOutputSchema = z.object({
  persona: z
    .string()
    .describe(
      "A detailed persona for the custom bot, including its scope of knowledge and rules for handling out-of-scope questions."
    ),
});
...
const createCustomBotFlow = ai.defineFlow(
  ...
  async (input) => {
    try {
      ...
      const context =
        input.field || input.profession || input.topic || "General Knowledge";
