...
function PaystackButton({ isAnnual, onSuccessfulPayment }: { isAnnual: boolean, onSuccessfulPayment: () => void }) {
    const { user } = useAuth();
  ...
  return (
    <div className="flex min-h-screen w-full flex-col items-center bg-background p-4 pt-12 md:pt-20">
      ...
      <div className="flex items-center justify-center space-x-2 md:space-x-3 mb-8">
        <Label htmlFor="billing-cycle" className={cn("font-medium", !isAnnual && "text-primary")}>Monthly</Label>
        <Switch id="billing-cycle" checked={isAnnual} onCheckedChange={setIsAnnual} />
        <Label htmlFor="billing-cycle" className={cn("font-medium", isAnnual && "text-primary")}>Annual</Label>
      ...
      <div className="grid w-full max-w-4xl gap-8 md:grid-cols-2">
        {plans.map((p) => {
          ...
          <Card
            key={p.id}
            className={`flex flex-col ${isCurrent && isPro ? 'border-primary ring-2 ring-primary' : ''}`}
          >
            <CardHeader>
            ...
            <CardFooter>
              {isCurrent ? (
                ...
                isUpgrading || subscriptionLoading ? (
                    ...
                    <PaystackButton isAnnual={isAnnual} onSuccessfulPayment={handleSuccessfulPayment} />
                )
              ) : null}
